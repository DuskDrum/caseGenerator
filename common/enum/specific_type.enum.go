...
var (
	// SPECIFIC_TYPE_BOOL 布尔字面量: true 和 false 被视为字符串（string），因此在解析时，它们的类型被标记为 token.STRING。
	SPECIFIC_TYPE_BOOL          = SpecificType{Name: "Bool", Type: token.STRING, ZeroValue: lo.Empty[bool](), Desc: "布尔类型"}
	SPECIFIC_TYPE_INT           = SpecificType{Name: "Int", Type: token.INT, ZeroValue: lo.Empty[int](), Desc: "Int"}
	SPECIFIC_TYPE_INT8          = SpecificType{Name: "Int8", Type: token.INT, ZeroValue: lo.Empty[int8](), Desc: "Int8"}
	SPECIFIC_TYPE_INT16         = SpecificType{Name: "Int16", Type: token.INT, ZeroValue: lo.Empty[int16](), Desc: "Int16"}
	SPECIFIC_TYPE_INT32         = SpecificType{Name: "Int32", Type: token.INT, ZeroValue: lo.Empty[int32](), Desc: "Int32"}
	SPECIFIC_TYPE_INT64         = SpecificType{Name: "Int64", Type: token.INT, ZeroValue: lo.Empty[int64](), Desc: "Int64"}
	SPECIFIC_TYPE_UINT          = SpecificType{Name: "Uint", Type: token.INT, ZeroValue: lo.Empty[uint](), Desc: "Uint"}
	SPECIFIC_TYPE_UINT8         = SpecificType{Name: "Uint8", Type: token.INT, ZeroValue: lo.Empty[uint8](), Desc: "Uint8"}
	SPECIFIC_TYPE_UINT16        = SpecificType{Name: "Uint16", Type: token.INT, ZeroValue: lo.Empty[uint16](), Desc: "Uint16"}
	SPECIFIC_TYPE_UINT32        = SpecificType{Name: "Uint32", Type: token.INT, ZeroValue: lo.Empty[uint32](), Desc: "Uint32"}
	SPECIFIC_TYPE_UINT64        = SpecificType{Name: "Uint64", Type: token.INT, ZeroValue: lo.Empty[uint64](), Desc: "Uint64"}
	SPECIFIC_TYPE_UINTPTR       = SpecificType{Name: "Uintptr", Type: token.INT, ZeroValue: nil, Desc: "uintptr是一个整数类型，用于存放一个指针值。实现与指针相关的算术运算：可以对uintptr类型的值进行算术运算，例如加上或减去一个偏移量，以便在内存中进行指针操作。这在一些底层编程场景中很有用，例如与内存映射、不安全的指针操作等结合使用。辅助垃圾回收器：在某些情况下，uintptr类型的值可以帮助垃圾回收器确定对象是否可达。但需要小心使用，因为不正确的使用可能导致错误的垃圾回收行为。需要注意的是，使用uintptr类型需要谨慎，因为它涉及到底层的指针操作，可能会导致不安全的行为和错误。在一般的 Go 编程中，应该尽量避免直接使用uintptr类型，除非你非常清楚自己在做什么并且有特定的底层编程需求。uintptr 是可以使用>、<进行比较的。在 Go 语言中，一旦将整数值存储在uintptr类型的变量中，无法直接获取回原始的整数值形式，因为uintptr类型主要用于指针相关的操作，并不直接对应一个整数值的表示形式。如果想要获取原始的整数值，可以考虑在将整数值转换为uintptr之前，将整数值存储在另一个变量中，或者在转换时记录下原始整数值以便后续使用。"}
	SPECIFIC_TYPE_FLOAT32       = SpecificType{Name: "Float32", Type: token.FLOAT, ZeroValue: lo.Empty[float32](), Desc: "Float32"}
	SPECIFIC_TYPE_FLOAT64       = SpecificType{Name: "Float64", Type: token.FLOAT, ZeroValue: lo.Empty[float64](), Desc: "Float64"}
	SPECIFIC_TYPE_COMPLEX64     = SpecificType{Name: "Complex64", Type: token.FLOAT, ZeroValue: lo.Empty[complex64](), Desc: "complex64是一种复合类型，表示由两个 32 位浮点数组成的复数类型。不支持使用>, <, ==, !=等比较运算符进行直接比较。不过他的实部和虚部可以比较 real(c1) == real(c2) && imag(c1) == imag(c2)。complex64类型通常在需要进行复数运算的科学计算、信号处理等领域中使用。"}
	SPECIFIC_TYPE_COMPLEX128    = SpecificType{Name: "Complex128", Type: token.FLOAT, ZeroValue: lo.Empty[complex128](), Desc: "complex128是一种复合类型，表示由两个 64 位浮点数组成的复数类型。complex128类型也不支持使用 ==, !=等比较运算符进行直接比较。通常在高精度的科学计算、工程计算、数学领域等需要处理复数的场景中使用。"}
