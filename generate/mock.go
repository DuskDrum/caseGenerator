...
func MockMatchConditionValue(op *token.Token, value any) any {
	...
	case token.EQL:
		return value
	case token.NEQ:
		return ProcessNotEqualValue(value)
	case token.LEQ:
		fallthrough
	case token.LSS:
...
func ProcessNotEqualValue(value any) any {
	...
	case reflect.Uintptr:
		...
		// 将指针转换为 uintptr 类型
		return uintptr(ptr)
	// complex64是一种复合类型，表示由两个 32 位浮点数组成的复数类型。不支持使用>, <, ==, !=等比较运算符进行直接比较
	// 不过他的实部和虚部可以比较 real(c1) == real(c2) && imag(c1) == imag(c2)
	// complex64类型通常在需要进行复数运算的科学计算、信号处理等领域中使用。
	case reflect.Complex64:
		panic("still not support complex64")
	// complex128是一种复合类型，表示由两个 64 位浮点数组成的复数类型。
	// complex128类型也不支持使用 ==, !=等比较运算符进行直接比较
	...
	// map 是引用类型, == 和 !=，只有当两个 map 的类型完全相同，并且具有相同的键值对时，它们才相等
	case reflect.Map:
		if value == nil {
...
func ProcessLessValue(value any) any {
	...
	case reflect.Complex64:
		panic("still not support complex64")
	// complex128是一种复合类型，表示由两个 64 位浮点数组成的复数类型。不支持使用>, <, ==, !=等比较运算符进行直接比较
	// 通常在高精度的科学计算、工程计算、数学领域等需要处理复数的场景中使用。
	case reflect.Complex128:
		panic("still not support complex128")
	// array是数组类型
